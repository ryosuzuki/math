...
sampleId = '7'
path = 'public/sample'
image = cv2.imread(path + '/sample-' + sampleId + '.jpg', 0)
...
cv2.imwrite(path + '/figure-' + sampleId + '.jpg', cv2.bitwise_not(image2))

# Create SVG based on lines
_, image2 = cv2.threshold(image2, 128, 255, cv2.THRESH_BINARY)
polys = trace_skeleton.from_numpy(image2)
dwg = svgwrite.Drawing(path + '/figure-' + sampleId + '.svg', size=(str(image2.shape[1])+'px', str(image2.shape[0])+'px'))
for poly in polys:
